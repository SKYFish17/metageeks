...
function wdsMobileMenu() {
	...
	function maybeOpenSubmenu( parent, subMenu ) {
		if ( parent.classList.contains( 'now-visible' ) ) {
			closeSubmenu( parent, subMenu );
			return;
		}

		// Expand the list menu item, and set the corresponding button aria to true.
		parent.classList.add( 'now-visible' );
		parent
			.querySelector( '.parent-indicator' )
			.setAttribute( 'aria-expanded', true );

		// Slide the menu in.
		subMenu.classList.add( 'now-visible' );
		( function ( $ ) {
	...
	function closeSubmenu( parent, subMenu ) {
		parent.classList.remove( 'now-visible' );
		parent
			.querySelector( '.parent-indicator' )
			.setAttribute( 'aria-expanded', false );
		subMenu.classList.remove( 'now-visible' );
		( function ( $ ) {
